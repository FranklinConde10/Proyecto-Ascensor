...
public class TramitealumnoController {
    ...
    @GetMapping ("")
    Page<Tramitealumno> index(@PageableDefault(sort = "tramitealumno", size = 5) Pageable pageable) {
    ...
    Tramitealumno actualizar(@PathVariable Integer id ,@RequestBody TramitealumnoDTO tramitealumnoDTO){
       Tramitealumno tramitealumno=tramitealumnoRepository.findById(id).orElseThrow(EntityNotFoundException::new);
